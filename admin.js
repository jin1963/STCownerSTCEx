(() => {
  ...
  async function refreshAll() {
    ...
    // auto-fill inputs with current params
  ...
  async function onChangeOwner() {
    try {
      if (!isOwner) return setStatus("คุณไม่ใช่ Owner ปัจจุบัน");

      const newOwner = ($("newOwnerAddr").value || "").trim();
      if (!newOwner || !newOwner.startsWith("0x") || newOwner.length !== 42) {
        throw new Error("กรุณาใส่ address ใหม่ให้ถูกต้อง");
      }

      const confirmMsg =
        "⚠️ ยืนยันเปลี่ยน Owner?\n\n" +
        "Owner ใหม่จะควบคุม contract ทั้งหมดทันที\n" +
        "คุณจะไม่สามารถเรียก setParams / withdraw / change owner ได้อีก\n\n" +
        "กด OK เพื่อยืนยัน";
  ...
  window.addEventListener("load", () => {
    ...
    $("btnChangeOwner").onclick = onChangeOwner;
